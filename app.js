...
window.abrirEditarNota = async (id) => {
    ...
    container.innerHTML = detalles.map((d, index) => `
        <div class="row g-2 mb-2 edit-item-row">
            <div class="col-6"><input type="text" class="form-control bg-light border-0" value="${d.modelo}" readonly></div>
            <div class="col-2"><input type="number" class="form-control edit-cant" value="${d.cantidad}" oninput="calcularTotalEdit()"></div>
            <div class="col-4"><input type="number" class="form-control edit-precio" value="${d.precio}" oninput="calcularTotalEdit()"></div>
        </div>
    `).join('');
...
window.actualizarNota = async () => {
    ...
    // Actualizar total en la tabla notas
    await fetch(`${SB_URL}/notas?id=eq.${id}`, { 
        method: 'PATCH', 
        headers, 
    ...
    Swal.fire('Actualizado', 'La nota ha sido modificada con éxito', 'success');
    fetchHistorial();
};
...
window.eliminarNota = async (id) => {
    const result = await Swal.fire({
        title: '¿Eliminar nota?',
        text: "Se borrará el historial de esta venta",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'Sí, eliminar'
    });
    ...
    if (result.isConfirmed) {
        ...
        fetchHistorial();
        Swal.fire('Borrado', 'Nota eliminada', 'success');
    }
};
...
window.guardarNota = async () => {
    ...
    if(!clienteId || total <= 0) return Swal.fire('Incompleto', 'Selecciona cliente y productos', 'info');
...
function renderTablas() {
    document.getElementById('tablaClientesBody').innerHTML = appData.clientes.map(c => `<tr><td class="px-4">${c.nombre}</td><td>${c.telefono || ''}</td><td class="text-end px-4"><button class="btn btn-sm btn-outline-primary me-1" onclick="abrirModalCliente('${c.id}')"><i class="bi bi-pencil"></i></button><button class="btn btn-sm btn-outline-danger" onclick="eliminarRegistro('clientes','${c.id}')"><i class="bi bi-trash"></i></button></td></tr>`).join('');
    document.getElementById('tablaModelosBody').innerHTML = appData.modelos.map(m => `<tr><td class="px-4">${m.nombre}</td><td class="text-end px-4"><button class="btn btn-sm btn-outline-primary me-1" onclick="abrirModalModelo('${m.id}')"><i class="bi bi-pencil"></i></button><button class="btn btn-sm btn-outline-danger" onclick="eliminarRegistro('modelos','${m.id}')"><i class="bi bi-trash"></i></button></td></tr>`).join('');
}
// (Funciones de abrirModalCliente y Modelo se mantienen del código anterior)
